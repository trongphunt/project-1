...
class My_Design_Endpoint {
    ...
    public function nbd_gallery_func($atts, $content = null) {
        ...
        $atts['templates'] = $this->nbdesigner_get_templates_by_page($page, absint($atts['row']), absint($atts['per_row']), $pid, false, false, $cat, $tag, $color);
        ob_start();
    ...
    public function nbd_save_for_later(){
        ...
        if( isset( $_POST['pre_folder'] ) ){
            $did = $this->get_design_by_folder( $_POST['pre_folder'] );
            if( $did ) {
    ...
    public static function nbdesigner_get_templates_by_page($page = 1, $row = 5, $per_row = 3, $pid = false, $get_all = false, $user_id = false, $cat = false, $tag = '', $color = '' ){
        $listTemplates = array();
        global $wpdb;
        $limit = $row * $per_row;
        $offset = $limit * ($page -1);
